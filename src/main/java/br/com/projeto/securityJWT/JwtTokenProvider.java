...
import br.com.projeto.exceptions.InvalidJwtAuthenticationException;
...
public class JwtTokenProvider {
	...
	public TokenVO refreshToken(String refreshToken) {
		// gera um novo token de forma automatica, caso o accessToken tenha expirado E o refreshToken seja valido! 
	...
	public DecodedJWT decodedJWT(String refreshToken) {
	...
	private String getAccessToken(String username, List<String> roles, Date now, Date validity) {
		String issuerUrl = ServletUriComponentsBuilder
				.fromCurrentContextPath().build().toUriString(); // obtem a url
		return JWT.create()
	...
	private String getRefreshToken(String username, List<String> roles, Date now) {
		Date validityRefreshToken = new Date(now.getTime() + (validityInMilliseconds * 3)); // mais tres horas!
		return JWT.create()
				.withClaim("roles", roles)
				.withIssuedAt(now)
